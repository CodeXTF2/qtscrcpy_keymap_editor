...
class KeyMapEditor(tk.Tk):
    def __init__(self):
        ...
        file_menu.add_command(label="Open Background Image", command=self.load_background_image)

        # Bind mouse events
        self.canvas.bind("<Button-1>", self.on_click)
        self.canvas.bind("<B1-Motion>", self.on_drag)
        self.canvas.bind("<Button-3>", self.on_right_click)  # Right-click event for menu
        ...
        if self.keymap:
            ...
            if file_path:
                with open(file_path, 'w') as file:
                    json.dump(self.keymap, file, indent=4)
    ...
    def render_key(self, keymap_node):
        x = keymap_node['pos']['x'] * 1920  # Scale to 1920x1080
        y = keymap_node['pos']['y'] * 1080

        # Create the key rectangle
        widget = self.canvas.create_rectangle(x-20, y-20, x+20, y+20, fill='blue')
        ...
        outline_ids = [
            self.canvas.create_text(x - 1, y - 25 - 1, text=key_label, font=("Arial", 10), fill="white"),  # Top-left
            self.canvas.create_text(x + 1, y - 25 - 1, text=key_label, font=("Arial", 10), fill="white"),  # Top-right
            self.canvas.create_text(x - 1, y - 25 + 1, text=key_label, font=("Arial", 10), fill="white"),  # Bottom-left
            self.canvas.create_text(x + 1, y - 25 + 1, text=key_label, font=("Arial", 10), fill="white"),  # Bottom-right
        ]

        # Create the main black text in front
        text_id_black = self.canvas.create_text(x, y-25, text=key_label, font=("Arial", 10), fill="black")

        self.key_widgets.append((widget, keymap_node, text_id_black, outline_ids))  # Store widget, black text, and outlines


    def render_steer_wheel(self, keymap_node):
        # Handle the special case for KMT_STEER_WHEEL (WASD control)
        x = keymap_node['centerPos']['x'] * 1920
        y = keymap_node['centerPos']['y'] * 1080
        widget = self.canvas.create_oval(x-50, y-50, x+50, y+50, outline='green', width=2)
        text_id = self.canvas.create_text(x, y-70, text="Steer Wheel (WASD)", font=("Arial", 10), fill="black")
        self.key_widgets.append((widget, keymap_node, text_id, 50))  # Include oval radius for later use

    def on_click(self, event):
        # Check if a key was clicked
        for widget, keymap_node, text_id, *extra in self.key_widgets:
            coords = self.canvas.coords(widget)
            if coords and coords[0] <= event.x <= coords[2] and coords[1] <= event.y <= coords[3]:
                self.selected_key = (widget, keymap_node, text_id, *extra)
                break


    def on_drag(self, event):
        if self.selected_key:
            widget, keymap_node, text_id_black, outline_ids, *extra = self.selected_key
            
            if keymap_node['type'] == "KMT_STEER_WHEEL":
                radius = extra[0]
                # Move the steering wheel and its label
                self.canvas.coords(widget, event.x - radius, event.y - radius, event.x + radius, event.y + radius)
                self.canvas.coords(text_id_black, event.x, event.y - 70)
                for outline_id in outline_ids:
                    self.canvas.coords(outline_id, event.x, event.y - 70)
                
                # Update the keymap's centerPos
                keymap_node['centerPos']['x'] = event.x / 1920
                keymap_node['centerPos']['y'] = event.y / 1080
            else:
                # Move the key and its text outline
                self.canvas.coords(widget, event.x - 20, event.y - 20, event.x + 20, event.y + 20)
                self.canvas.coords(text_id_black, event.x, event.y - 25)
                # Move the white outline around the black text
                self.canvas.coords(outline_ids[0], event.x - 1, event.y - 25 - 1)  # Top-left
                self.canvas.coords(outline_ids[1], event.x + 1, event.y - 25 - 1)  # Top-right
                self.canvas.coords(outline_ids[2], event.x - 1, event.y - 25 + 1)  # Bottom-left
                self.canvas.coords(outline_ids[3], event.x + 1, event.y - 25 + 1)  # Bottom-right
                
                # Update the keymap's pos
                keymap_node['pos']['x'] = event.x / 1920
                keymap_node['pos']['y'] = event.y / 1080
    ...
    def show_modify_delete_menu(self, clicked_widget):
        ...
        if keymap_node['type'] == "KMT_STEER_WHEEL":
            ...
            modify_button = tk.Button(dialog, text="Modify Keys", command=modify_steer_wheel_keys, width=20)
            modify_button.pack(pady=5)
        else:
            # Modify Key Button
            def modify_key():
                new_key = simpledialog.askstring("Change Key Binding", "Enter new key:")
                if new_key:
                    keymap_node['key'] = new_key
                    self.canvas.itemconfig(text_id, text=new_key)
                dialog.destroy()

            modify_button = tk.Button(dialog, text="Modify Key", command=modify_key, width=20)
            modify_button.pack(pady=5)

        # Delete Key Button
        def delete_key():
            self.keymap['keyMapNodes'].remove(keymap_node)
            self.key_widgets = [w for w in self.key_widgets if w[1] != keymap_node]  # Remove from key_widgets
            self.canvas.delete(widget)
            self.canvas.delete(text_id)
            dialog.destroy()

        delete_button = tk.Button(dialog, text="Delete Key", command=delete_key, width=20)
    ...
    def add_key(self, x, y):
        ...
        if key_type == "KMT_STEER_WHEEL":
            ...
            key_value = simpledialog.askstring("Add Key Binding", "Enter key (e.g., Key_W):")
            if not key_value:
                return
    ...
    def ask_key_type(self):
        ...
        button_click = tk.Button(dialog, text="KMT_CLICK", command=select_kmt_click, width=20)
        button_click.pack(pady=5)

        button_steer_wheel = tk.Button(dialog, text="KMT_STEER_WHEEL", command=select_kmt_steer_wheel, width=20)
